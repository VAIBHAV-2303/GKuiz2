...
class MovieQuiz extends Component {
  constructor() {
    super();
    this.state = {
      data: [],
  ...
  handleFilter(event) {
  	...
  	this.state.attemptedData.map(function(item,  key){
  		if(item.personId==Number(window.localStorage.getItem('currentUserId'))){
  			if(item.category=="movies"){
  				// console.log("pui", item.quizNumber, reqfilter);
  ...
  handleSubmit(event){
  	var tempthis = this
  	this.state.data.map(function(item, key) {
  		if(item.category=="movies" && item.quizNumber==tempthis.state.curQuizNumber){
  			if(item.typeOf=="single"){
  				if(tempthis.getRadioVal("selectedAnswer"+item.id)==item.ans){
  					tempthis.state.correct+=1;
  					tempthis.state.formData.movieScore+=1;
  				...
  				// console.log(Number(document.getElementById("selectedAnswer1"+item.id).checked), Number(item.ans1), Number(document.getElementById("selectedAnswer2"+item.id).checked), Number(item.ans2), Number(document.getElementById("selectedAnswer3"+item.id).checked), Number(item.ans3));
    ...
    window.localStorage.setItem('currentUserMovieScore', this.state.formData.movieScore.toString());
    ...
    return (
      <div className="App">
        <header className="App-header">
          <h1 className="App-title">Answer movie questions</h1>
        </header>
